...
int tcp_send_data(struct tcp_sock *tsk, char *buf, int len)
{
	...
	if (!packet) {
		log(ERROR, "malloc failed in tcp_send_data.");
		return -1;
	}
...
void tcp_send_control_packet(struct tcp_sock *tsk, u8 flags)
{
	...
	printf("[DEBUG] send_control_packet: flags=0x%x, seq=%u, ack=%u, len=%d -> "IP_FMT":%hu\n",
		flags, tsk->snd_nxt, tsk->rcv_nxt, 
		NET_IP_FMT_STR(tsk->sk_dip), ntohs(tsk->sk_dport));

}
void tcp_send_data_packet(struct tcp_sock *tsk, const char *data, int len)
{
	...
	memcpy(payload, data, len); // 拷贝你传进来的 payload

	u16 tot_len = IP_BASE_HDR_SIZE + TCP_BASE_HDR_SIZE + len;

	ip_init_hdr(ip, tsk->sk_sip, tsk->sk_dip, tot_len, IPPROTO_TCP);
	tcp_init_hdr(tcp, tsk->sk_sport, tsk->sk_dport, tsk->snd_nxt,
	             tsk->rcv_nxt, TCP_ACK, tsk->rcv_wnd);

	tcp->checksum = tcp_checksum(ip, tcp); // 注意要计算 checksum 时包含 payload
