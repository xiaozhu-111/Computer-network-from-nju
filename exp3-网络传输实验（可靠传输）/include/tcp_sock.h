...
#define TCP_MAX_BACKLOG 128
	...
	struct synch_wait *wait_send1;
	struct synch_wait *wait_recv1;
	...
	//自创
	pthread_mutex_t sk_lock;         // 保护 snd_una、snd_nxt 等核心参数
    pthread_mutex_t rcv_buf_lock;    // 保护接收缓冲区 rcv_buf
    pthread_mutex_t send_buf_lock;   // 保护发送缓冲区 send_buf

	struct tcp_timer persist_timer;  // 用于 Persist Timer 实现
};
typedef struct tcp_sock tcp_sock;  // 在结构体定义后添加
